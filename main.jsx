...
function Dashboard({user,members,openMember,onAdd}){
 ...
 return <div className="page">
   <div className="hero"><div><span className="eyebrow">Assalamu Alaikum</span><h2>{user.masjidName||'Your Masjid'}</h2><p>{user.village||'Village / Area'} • Monthly collection manager</p></div><div className="hero-icon">🕌</div></div>
   <div className="stats">
...
function SettingsPage({user,onSave}){const [f,setF]=useState({...user});return <div className="page narrow"><span className="eyebrow">Account</span><h2>Settings</h2><p className="sub">Masjid aur account details manage karein.</p><div className="form-card"><Field icon={<UserRound/>} label="Owner Name" value={f.name||''} placeholder="Name" onChange={v=>setF({...f,name:v})}/><Field icon={<Smartphone/>} label="Mobile Number" value={f.mobile||''} placeholder="Mobile" onChange={v=>setF({...f,mobile:v})}/><Field icon={<MapPin/>} label="Village / Area" value={f.village||''} placeholder="Village" onChange={v=>setF({...f,village:v})}/><Field icon={<Building2/>} label="Masjid Name" value={f.masjidName||''} placeholder="Masjid" onChange={v=>setF({...f,masjidName:v})}/><Field icon={<IndianRupee/>} label="Default Monthly Fee" value={f.defaultFee||500} placeholder="500" onChange={v=>setF({...f,defaultFee:v})}/><button className="primary-btn" onClick={()=>onSave(f)}>Save Settings <CheckCircle2/></button></div></div>}
